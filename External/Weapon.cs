...
namespace RedM.External
{
    public sealed class Weapon
    {
        ...
        public bool IsPresent => Hash == WeaponHash.Unarmed || Function.Call<bool>(CitizenFX.Core.Native.Hash.HAS_PED_GOT_WEAPON, Owner.Handle, Hash, false, false);

        public Model Model => GetModelByHash(Hash);

        public WeaponGroup Group => Function.Call<WeaponGroup>(CitizenFX.Core.Native.Hash.GET_WEAPONTYPE_GROUP, (uint)Hash);

        public AmmoType AmmoType => Function.Call<AmmoType>(CitizenFX.Core.Native.Hash.GET_PED_AMMO_TYPE_FROM_WEAPON, Owner.Handle, (uint)Hash);

        public int Ammo
        {
            get => Hash == WeaponHash.Unarmed
                ? 1
                : Function.Call<int>(CitizenFX.Core.Native.Hash.GET_PED_AMMO_BY_TYPE, Owner.Handle, AmmoType);
            set {
                if (IsPresent)
                {
                    Function.Call(CitizenFX.Core.Native.Hash.SET_PED_AMMO_BY_TYPE, Owner.Handle, AmmoType, value);
                }
                else
                {
        ...
        public int AmmoInClip
        {
            get {
                ...
                if (Function.Call<bool>(CitizenFX.Core.Native.Hash.GET_AMMO_IN_CLIP, Owner.Handle, (uint)Hash, ammo))
                {
            ...
            set {
                ...
                if (IsPresent)
                {
                    Function.Call(CitizenFX.Core.Native.Hash.SET_AMMO_IN_CLIP, Owner.Handle, (uint)Hash, value);
                }
                else
                {
                    Owner.GiveWeapon(Hash, value);
        ...
        public int MaxAmmo
        {
            get {
                ...
                var ammo = new OutputArgument();
                return Function.Call<bool>(CitizenFX.Core.Native.Hash.GET_MAX_AMMO, Owner.Handle, (uint)Hash, ammo) ? ammo.GetResult<int>() : 1;
            }
        }

        public int MaxAmmoInClip => Hash == WeaponHash.Unarmed ? 1 : Function.Call<int>(CitizenFX.Core.Native.Hash.GET_MAX_AMMO_IN_CLIP);

        public bool InfiniteAmmo
        {
            set => Function.Call(CitizenFX.Core.Native.Hash.SET_PED_INFINITE_AMMO, Owner.Handle, (uint)Hash, value);
        }

        public void Remove()
        {
            Function.Call(CitizenFX.Core.Native.Hash.REMOVE_WEAPON_FROM_PED, Owner.Handle, (uint)Hash, false, false);
        }
